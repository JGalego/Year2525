...
There is also a presenter mode, on the screen icon in the header, which walks the same gallery as slides — arrow keys, space, or the pill at the bottom; escape to leave, and you land back at the exhibit you were on. It opens on the gallery's own placard, then breaks each forward era into a title slide, one slide per passage, and a closing slide for its widget, which stays live and usable on the slide. The built slides all wear the same three parts in the same places: an eyebrow naming the room, a heading naming the passage, and the prose. Every slide fits on one screen: rather than picking a type size and hoping, a slide is laid out in a virtual viewport and scaled to the real one by whatever factor makes it fit, so nothing scrolls off the bottom and nothing on the site had to be shortened to make room. Slides with space left over scale up into it instead.

The printer icon in that same pill saves all 134 slides as a PDF, one 16:9 page each (Ctrl/Cmd+P does it too, while presenter mode is open). There is no library and no server behind it: the browser's own print pipeline does the writing, so the export's job is to hand it a document worth printing. It walks the gallery once to catch every widget at its mounted size, fits each page by the same measurement the screen uses, re-emits the theme cascade against the pages so each room prints in its own colours, and photographs the era dioramas — whose blur filters, left as vectors, are the difference between a 58MB file and a 13MB one. Choose "Save as PDF" in the dialog and leave the paper size on the one the page asks for. Printing the site normally, from the browser's own menu, is unaffected.
...
- `js/main.js` — scroll-driven era theming, the year readout, dot nav, typed easter eggs, presenter mode, its slide fitter, and the PDF export
